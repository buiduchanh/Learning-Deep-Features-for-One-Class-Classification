...
def calculateDistance(x1,y1,x2,y2):  
    ...
    return dist 
def cut_center_image(image, flag_draw_line=False, minLineLength=None, margin_min=10, margin_max=10, abs_x = 14, abs_y = 14):
    ...
    for line in lines:
        ...
        if abs(x1 - x2) < abs_x and x1 > margin_min and x2 < w - margin_min:
#                 vertical
            if flag_draw_line:
        ...
        if abs(y1 - y2) < abs_y and y1 > margin_min and y2 < h - margin_min:
#                 horizontal
            if flag_draw_line:
                cv2.line(image, (x1, y1), (x2, y2), (0, 255,255), 3)
...
def cut_top_bottom(image, flag_draw_line=False, minLineLength=None, margin_min=0, margin_max=0, abs_x = 20, abs_y = 20):
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    img_hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
    lower_yellow = np.array([20, 100, 100], dtype = 'uint8')
    upper_yellow = np.array([30, 255, 255], dtype='uint8')
...
#     print(vertical_z_min)
    ...
    horizontal_z_bk = horizontal_z.copy()
...
#     print(horizontal_z_min)
    ...
    margin = 30
    horizontal_z = horizontal_z[horizontal_z[:, 0] > (horizontal_z_min[0] + margin)]
    horizontal_z = horizontal_z[horizontal_z[:, 1] > (horizontal_z_min[1] + margin)]
...
#     print(vertical_z_min)
    ...
    if y_min > h/3:
        horizontal_z_min = horizontal_z_bk.min(axis=0)
        y_min = int(horizontal_z_min[1]) + margin_min
...
#     cv2.rectangle(image, (x_max, y_max), (h, w), (255,0,0), 2)
    ...
    return edges, cut_top_bottom_image, (x1, y1, x2, y2)
def crop_save_image_font(image):
    cut_top_bottom_edges, cut_top_bottom_image, rec_1 = \
        cut_top_bottom(image, flag_draw_line=False, minLineLength=image.shape[0]/4)
    x1 = rec_1[0]
    y1 = rec_1[1]
    x2 = rec_1[2]
    y2 = rec_1[3]
    # cv2.rectangle(image, (x1, y1), (x2, y2), (255,0,0), 2)
    # show_images([image, cut_top_bottom_edges, cut_top_bottom_image], cols = 1)
    edge_image = cut_top_bottom_image.copy()
    _ , edges, rec_2 = cut_center_image(edge_image, flag_draw_line=False, minLineLength=edge_image.shape[0]/4)
    # x1 = int(rec_2[0] + (rec_2[0]-rec_1[0])/2)
    # y1 = int(rec_2[1] + (rec_2[1]-rec_1[1])/2)
    # x2 = int(rec_2[2] + (rec_2[2]-rec_1[2])/2)
    # y2 = int(rec_2[3] + (rec_2[3]-rec_1[3])/2)
